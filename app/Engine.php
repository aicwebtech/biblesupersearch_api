...
class Engine implements ErrorInterface
{
    ...
    protected function _renderDownloadHelper($input, $action = 'render') 
    {
        ...
        if($action == 'render_needed') {
            ...
            //
            // It is not a public capability. The request may ask, but only an
            // authenticated administrator is granted it, and only for a
            // genuinely true value.
            //
            // The download widget submits it unconditionally and its JavaScript
            // sets it back to 0 once that JavaScript runs, so the callers who
            // actually reach here asking for it are an administrator and a
            // visitor with JavaScript off. The latter is held to the limit like
            // any other caller and falls back to the queued-render path in
            // RenderManager::getBiblesNeedingRender().
