...
use App\Exceptions\RenderStageFailedException;
...
abstract class RenderAbstract 
{
    ...
    public function render($overwrite = FALSE, $suppress_overwrite_error = FALSE) 
    {
        ...
        try {
            try {
                ...
                // A stage may report failure by returning falsy rather than by throwing, and
                // the two mean the same thing here. Raised so the cleanup below runs either
                // way: this early return used to sit inside the try and skip it, leaving the
                // destination TextAbstract has already truncated in place, with the handle
                // still open and the previous render's metadata intact - the artifact
                // _onRenderError() exists to discard.
                if(!$success) {
                    throw new RenderStageFailedException('_renderStart');
                }
                ...
                // Same again, and it costs more here: a falsy $success used to fall through to
                // the bookkeeping below, which stamps the Rendering record with a fresh
                // rendered_at, version and meta_hash before returning FALSE. isRenderNeeded()
                // then reported FALSE and download() handed the unfinished file out as the
                // current render.
                if(!$success) {
                    throw new RenderStageFailedException('_renderFinish');
                }
            }
            catch(\Throwable $e) {
        ...
        catch(RenderStageFailedException $e) {
            // Cleaned up above, and answered the way a refused render has always been answered:
            // render() returns FALSE rather than throwing, which is what lets RenderManager go
            // on to the next Bible. Recorded rather than returned bare, so the caller has
            // something to report - a stage that refuses silently is why this was hard to see.
            return $this->addError($e->getMessage());
        }
        finally {
            // Neither the throws above nor the return can be allowed to skip this. Because
     ...
     *
     * Returning falsy refuses the render, and is treated exactly as a throw is: render() runs
     * _onRenderError() and answers its caller with FALSE.
     *
     ...
     *
     * Returning falsy refuses the render, and is treated exactly as a throw is: render() runs
     * _onRenderError(), leaves the Rendering record alone so the next call re-renders, and
     * answers its caller with FALSE.
     *
     * @return bool $success
     */
    ...
    /**
     * Code to be executed when any render stage fails, after _onVerseRenderError() where both
     * apply. Usage: releasing whatever _renderStart() acquired, and discarding the half-written
     * artifact.
     *
     * Reached whether the stage threw or returned falsy - render() raises
     * RenderStageFailedException for the latter so the two cannot come apart.
     *
     ...
     *
     * The exception is re-thrown afterwards, so this must not swallow it - render() catches
     * RenderStageFailedException itself and lets everything else out.
     *
     * @param \Throwable $e
     */
