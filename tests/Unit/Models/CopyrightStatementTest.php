...
class CopyrightStatementTest extends TestCase
{
    ...
    public function testAHostileUrlCannotBreakOutOfTheHref(string $url): void
    {
        ...
        $this->assertStatementHasNothingButTheIntendedLink($Copyright->getProcessedCopyrightStatement(NULL, TRUE));
    }
    ...
    public function testAnAmpersandInTheUrlIsEscaped(): void
    {
        ...
        $this->assertStringContainsString('a=1&amp;b=2', $Copyright->getProcessedCopyrightStatement(NULL, TRUE));
    }
    ...
    /**
     * The strict editor allowlist: wider than the API one, because a copyright statement is
     * written in the admin editor and RenderAbstract hands the result to TCPDF as HTML - the
     * rule, the strikethrough and the classes have to survive. Narrower than the editor
     * allowlist, because this is a public-facing value: it ships in the /api/bibles listing
     * and in every rendered file, and an image there is a third-party fetch for whoever opens
     * them. The editor itself reads the column back through Bible::copyrightStatement(),
     * which keeps the image - see ModelHtmlSanitizationTest.
     */
    public function testTheStatementKeepsEditorMarkupButNotTheImage(): void
    {
        $Bible = new Bible();
        $Bible->setRawAttributes(['copyright_statement' => '<p class="cr">Ok</p><img src="/logo.png"><hr><s>old</s>']);

        $statement = $Bible->getCopyrightStatement();

        $this->assertStringContainsString('<hr />', $statement);
        $this->assertStringContainsString('<s>old</s>', $statement);
        $this->assertStringContainsString('class="cr"', $statement);
        $this->assertStringNotContainsString('<img', $statement);
        $this->assertStringNotContainsString('/logo.png', $statement);
    }
